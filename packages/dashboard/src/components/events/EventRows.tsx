...
import type { Event, EventPayload, DerivedState } from "@substructure.ai/sdk/types";
...
function eventIcon(type: EventPayload["type"]): React.ReactNode {
    switch (type) {
        case "message.new":
            return <MessageSquare size={12} className={ICON_CLASS} />;
        case "session.created":
            return <Play size={12} className={ICON_CLASS} />;
        case "session.done":
            return <CheckCircle2 size={12} className="text-green-600 dark:text-green-400" />;
        case "session.cancelled":
            return <Square size={12} className="text-red-500" />;
        case "session.interrupted":
            return <Square size={12} className="text-amber-500" />;
        case "llm.call.requested":
            return <BrainCircuit size={12} className={ICON_CLASS} />;
        case "llm.call.completed":
            return <BrainCircuit size={12} className={ICON_CLASS} />;
        case "llm.call.errored":
            return <XCircle size={12} className="text-red-500" />;
        case "tool.call.requested":
            return <Wrench size={12} className={ICON_CLASS} />;
        case "tool.call.completed":
            return <Wrench size={12} className={ICON_CLASS} />;
        case "tool.call.errored":
            return <XCircle size={12} className="text-red-500" />;
        case "sub_agent.requested":
            return <Bot size={12} className={ICON_CLASS} />;
        ...
        case "worker.decision.requested":
            return <Zap size={12} className={ICON_CLASS} />;
        case "worker.decision.completed":
            return <Zap size={12} className={ICON_CLASS} />;
        case "worker.decision.errored":
            return <XCircle size={12} className="text-red-500" />;
        case "turn.started":
            return <RotateCcw size={12} className={ICON_CLASS} />;
        case "turn.completed":
            return <RotateCcw size={12} className={ICON_CLASS} />;
        default:
            return <Circle size={4} className="text-[var(--color-text-secondary)] fill-current" />;
    }
}
    ...
    return (
        <>
            {items.map((item) =>
                isGroup(item) ? (
                    <GroupRow
                        key={`group-${item.startEvent.aggregate_id}-${item.startEvent.sequence}`}
                        group={item}
                        depth={depth}
                    />
                ) : (
...
export const GroupRow = memo(function GroupRow({ group, depth = 0 }: { group: EventGroup; depth?: number }) {
    ...
    const subAgentSessionId =
        group.startEvent.payload.type === "sub_agent.requested" ? group.startEvent.payload.session_id : null;
...
function SubAgentGroup({ sessionId, depth = 0 }: { sessionId: string; depth?: number }) {
    ...
    return (
        <TreeRow
            ...
            trailing={<SubAgentTrailing items={items} isDone={isDone} />}
        >
            {isLoading ? (
                <div
                    className="flex items-center gap-2 py-2 text-xs text-[var(--color-text-secondary)]"
...
function SubAgentTrailing({ items, isDone }: { items: GroupedItem[]; isDone: boolean }) {
    ...
    const lastEvent = lastItem
        ? isGroup(lastItem)
            ? (lastItem.endEvent ?? lastItem.startEvent)
            : lastItem
        : undefined;

    const duration =
        firstEvent && lastEvent && isDone
    ...
    return (
        <>
            <span
                className={`text-xs font-mono whitespace-nowrap ${isDone ? "text-[var(--color-text-secondary)]" : "text-[var(--color-accent)]"}`}
            >
                {duration}
            </span>
