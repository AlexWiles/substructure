    ...
    return (
        <>
            ...
            >
                ...
                {summary && (
                    <span className="min-w-0 truncate text-xs text-[var(--color-text-secondary)]">{summary}</span>
                )}
                <span className="flex-1" />
                {trailing && <span className="flex items-center gap-2 shrink-0">{trailing}</span>}
            </button>
            {expanded && children}
        </>
    );
