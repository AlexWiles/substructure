...
function renderDetail(payload: EventPayload): React.ReactNode {
    switch (payload.type) {
        ...
        case "session.done":
            return <div className="text-sm text-green-600 dark:text-green-400 font-medium">Session completed</div>;
        ...
        case "sub_agent.requested":
            return (
                <DetailGrid>
                    <Field label="Agent">{payload.agent_id}</Field>
                    <Field label="Session">
                        <span className="font-mono text-xs">{payload.session_id}</span>
                    </Field>
                </DetailGrid>
            );
            ...
            return (
                <DetailGrid>
                    ...
                    {payload.token_usage && Object.keys(payload.token_usage).length > 0 && (
                        <Field label="Tokens">
                            {Object.entries(payload.token_usage)
                                .map(([k, v]) => `${k}: ${v}`)
                                .join(", ")}
                        </Field>
                    )}
        ...
        case "session.interrupted":
            return (
                <DetailGrid>
                    ...
                    <Field label="Interrupt ID">
                        <span className="font-mono text-xs">{payload.interrupt_id}</span>
                    </Field>
                </DetailGrid>
            );
        ...
        case "turn.started":
            return (
                <DetailGrid>
                    <Field label="Turn ID">
                        <span className="font-mono text-xs">{payload.turn_id}</span>
                    </Field>
                </DetailGrid>
            );
                    ...
                    <DetailGrid>
                        ...
                        {turn_token_usage && Object.keys(turn_token_usage).length > 0 && (
                            <Field label="Token Usage">
                                {Object.entries(turn_token_usage)
                                    .map(([k, v]) => `${k}: ${v}`)
                                    .join(", ")}
                            </Field>
                        )}
...
function detailHeader(payload: EventPayload): string | null {
    switch (payload.type) {
        case "message.new":
            return `${capitalize(payload.message.role)}:`;
        case "session.created":
        ...
        case "session.interrupted":
            return `Interrupted: ${payload.reason}`;
        case "llm.call.requested":
            return payload.request.model;
        case "llm.call.completed":
            return `${payload.response.model} · ${payload.response.finish_reason ?? "done"}`;
        case "llm.call.errored":
            return `LLM Error`;
        case "tool.call.requested":
            return payload.name;
        ...
        case "sub_agent.turn_completed":
            return `Sub-Agent Turn · $${payload.cost}`;
        case "sub_agent.errored":
            return `Sub-Agent Error`;
        case "worker.decision.requested":
            return `Decision: ${payload.trigger.type}`;
        case "worker.decision.errored":
            return `Decision Error`;
        case "turn.started":
            return `Turn Started`;
        case "turn.completed":
            return payload.turn_cost ? `Turn Completed · $${payload.turn_cost}` : `Turn Completed`;
        default:
            return null;
    }
}
