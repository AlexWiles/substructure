...
function SessionsPage() {
    ...
    return (
        <Page>
            <Breadcrumbs crumbs={[{ label: "sessions", to: "/sessions" }]} />
                    ...
                    <TBody>
                        {sessions.map((item: SessionListItem) => (
                            <tr
                                ...
                                onClick={() =>
                                    navigate({ to: "/sessions/$sessionId", params: { sessionId: item.session_id } })
                                }
                            >
                                <Td>
                                    <Link to="/sessions/$sessionId" params={{ sessionId: item.session_id }}>
                                        {item.session_id}
                                    </Link>
                                </Td>
                                <Td>{item.agent_id ?? "-"}</Td>
                                ...
                                <Td secondary>
                                    {item.last_event_at ? new Date(item.last_event_at).toLocaleString() : "-"}
                                </Td>
                            </tr>
                        ))}
                    </TBody>
