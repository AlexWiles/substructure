...
worker reads the decision request and returns the next actions. Two triggers make
a chat agent — the client proposes the conversation (`client.messages`) → prompt
the model; the model answers (`llm.finished`) → end the turn.

`Mix.install` pulls Bandit, Plug, and Jason, so the worker runs as a single
script with no project scaffolding. The worker contract is one JSON request in,
one JSON response out. See [`docs/07-protocol.md`](../../docs/07-protocol.md) for
the full protocol.
