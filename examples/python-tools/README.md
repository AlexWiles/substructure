...
A chattable agent with its own tool, in Python — no SDK, one FastAPI POST handler.
The whole worker reads the decision request and returns the next actions. It
accepts every decision the engine has a default for (`proposed`) first, then
authors the two that are genuinely its own: `client.messages` → the LLM request
(the agent's identity), and `tool.execute` → run the tool. Everything else — tool
results, model replies, model failures, even broken or hallucinated tool calls —
