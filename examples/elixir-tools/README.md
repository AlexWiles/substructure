...
A chattable agent with its own tool, in Elixir — no SDK, one Plug handler. The
...
script with no project scaffolding. The single tool here, `get_current_time`,
returns the current UTC time. For the same agent without tools, see
[`elixir-basic`](../elixir-basic).
