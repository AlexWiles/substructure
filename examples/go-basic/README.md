...
first, then authors the one that is genuinely its own: `client.messages` → the
LLM request (the agent's identity). Everything else — model replies and model
failures — is the engine's default loop, accepted by echoing `proposed` back.
