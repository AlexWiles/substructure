...
| `defer_tools` | bool | `false` by default. Keeps every tool of this agent out of the request, whatever its source. A tool or a connection overrides it with its own `defer`. See [Deferred tools](./65-deferred-tools.md). |
| `tool_search` | string | Which tools find the deferred ones: `full` (the default) or `search`. |
