...
status after two minutes, so a turn still thinking has it re-set. A message
queued behind a running turn has no turn of its own to report yet, so nothing
marks it until the turn before it settles.
