...
2. Emits a `sub_agent.spawn` action carrying the child's session id, the agent id, and the originating `tool_call_id`.
3. Emits a `message.send` action that delivers the LLM's `message` argument to the child as a user message.

The engine creates the child session and runs it to completion. The child's result (its `done` output, or its error if it failed past retries) returns to the parent as a `sub_agent.finished` trigger, folded in as a tool message on that `tool_call_id`, alongside any sibling tool results from the same turn. The loop keeps no correlation state; the engine threads the `tool_call_id` through the child's lifetime.
...
| `sub_agent.turn_completed` | Child finished a turn; the result will land back at the parent as a `sub_agent.finished` trigger. |
