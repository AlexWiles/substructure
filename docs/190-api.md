...
List an org's apps. Returns an array of [`App`](#app).
...
**Response** `200`. An [`App`](#app). `balanceUsd` and `sessionCount` are
...
```json
{ "endpointUrl": "https://my-worker.example.com/agent", "state": "enabled" }
```

**Response** `200`. The worker config, as `GET` returns.

### `POST /api/v1/apps/{app}/worker/rotate-secret`

Rotate the signing secret. No body; returns the worker config with the new
secret.
...
### `DELETE /api/v1/apps/{app}/api-keys/{key_id}`
