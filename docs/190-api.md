...
| `session.interrupted` / `.interrupt_resumed` | `{ interrupt_id, origin, reason, payload }`. |
| `worker.state.updated` / `agent.updated` | State or config was rewritten. |
| `worker.decision.requested` / `.completed` / `.errored`, `decision_request.queued` / `.dropped`, `control.new`, `call.voided`, `session.message_requested` | Engine bookkeeping, useful for observability. |
...
Pause a running session. `interrupt_id` is minted when omitted.
...
**Request body:** a [`RunAgentInput`](#runagentinput) (only `threadId` is read).

```json
{ "threadId": "sess-1", "runId": "turn-2" }
```
